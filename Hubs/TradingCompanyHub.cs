...
namespace Trading_company.Hubs
{
    ...
    public class TradingCompanyHub : Hub
    {
        ...
        public void GetCheck(string prod_id, string quantity, string transaction_date)
        {
            ...
            var list = _db.some_model.FromSqlInterpolated($"select {Convert.ToInt32(quantity)} * pc.value * (select value from cources where cur_idfrom = crc.cur_id and cur_idto = 1) cost from Products pd left join Prices pc on pd.prod_id = pc.prod_id left join Currencies crc on pc.cur_id = crc.cur_id left join Cources cr on crc.cur_id = cr.cur_idfrom where pd.prod_id = {Convert.ToInt32(prod_id)} and pc.dateto > {DateTime.Parse(transaction_date)}").ToList();
            ...
            cost = list.ToList()[0].summ;

            tax = _db.some_model.FromSqlInterpolated($"select value from Taxes where Tax_Id = 1").ToList()[0].summ * cost;
